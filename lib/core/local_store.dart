...
class AppRepository {
  ...
  bool get _isVehiclesCloudAvailable => FirebaseBootstrap.isInitialized;
  ...
  Future<List<VehicleRecord>> _readLocalVehiclesOnly({
  ...
  Future<void> _writeVehiclesCache(List<VehicleRecord> items) async {
  ...
  Future<List<VehicleRecord>> listVehicles({bool activeOnly = false}) async {
    final localItems = await _readLocalVehiclesOnly();
    if (!_isVehiclesCloudAvailable) {
      ...
      return localItems.where((item) => item.active).toList(growable: false);
    }

    try {
      var cloudItems = await _vehiclesCollection.get().then(
      ...
      if (localItems.isNotEmpty) {
        ...
        for (final item in localItems) {
          ...
          await _vehiclesCollection.doc(item.id).set(
                <String, dynamic>{
                  'user_id': localUserId,
                  ...item.toMap(),
                },
          ...
          cloudItems = <VehicleRecord>[...cloudItems, item];
      ...
      await _writeVehiclesCache(cloudItems);
      _lastVehiclesDataSourceLabel = 'cloud';
      _lastVehiclesFallbackReason = '';
      if (!activeOnly) {
        return cloudItems;
      }
      return cloudItems.where((item) => item.active).toList(growable: false);
    } catch (error) {
  ...
  Future<void> saveVehicle(VehicleRecord vehicle) async {
    var queuedOffline = !_isVehiclesCloudAvailable;
    if (_isVehiclesCloudAvailable) {
      try {
    ...
    if (queuedOffline) {
      await OfflineSyncRuntime.instance.queueVehicleUpsert(vehicle);
    }
  ...
  Future<void> deleteVehicle(String id) async {
    ...
    if (queuedOffline) {
      await OfflineSyncRuntime.instance.queueVehicleDelete(id);
    }
