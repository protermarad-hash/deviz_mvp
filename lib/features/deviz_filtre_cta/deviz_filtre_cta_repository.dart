import 'dart:convert';

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/foundation.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../../core/cloud/firebase_bootstrap.dart';
import '../../core/cloud/firebase_collections.dart';
import '../../core/cloud/offline_sync_runtime.dart';
import 'deviz_filtre_cta_models.dart';

/// Repository pentru devize filtre CTA — Firebase + cache local + offline queue.
/// Pattern obligatoriu (CLAUDE.md):
///   1. Scriere locală (funcționează offline)
///   2. Queue OBLIGATORIU (sincronizare automată la revenire internet)
///   3. Firebase direct (best-effort)
class DevizFiltreCtaRepository {
  static const _prefKey = 'devize_filtre_cta_cache';

  CollectionReference<Map<String, dynamic>> get _col =>
      FirebaseFirestore.instance
          .collection(FirebaseCollections.devizeFiltreCta);

  bool get _isCloudAvailable => FirebaseBootstrap.isInitialized;

  // ── Diagnostics ──────────────────────────────────────────────────────────
  static String? lastFirestoreError;
  static int lastFirestoreCount = -1;
  static int lastLocalCount = 0;

  // ── Cache local ──────────────────────────────────────────────────────────

  Future<List<DevizFiltreCta>> listLocal() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_prefKey);
      if (raw == null || raw.isEmpty) return [];
      final list = jsonDecode(raw) as List;
      return list
          .map((e) =>
              DevizFiltreCta.fromMap(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (_) {
      return [];
    }
  }

  Future<void> _saveLocal(List<DevizFiltreCta> records) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      _prefKey,
      jsonEncode(records.map((r) => r.toMap()).toList()),
    );
  }

  Future<void> _updateLocalCache(DevizFiltreCta record) async {
    final locals = await listLocal();
    final idx = locals.indexWhere((r) => r.id == record.id);
    if (idx >= 0) {
      locals[idx] = record;
    } else {
      locals.insert(0, record);
    }
    await _saveLocal(locals);
  }

  // ── List (cloud + local merge) ───────────────────────────────────────────

  Future<List<DevizFiltreCta>> list() async {
    final localItems = await listLocal();

    if (!_isCloudAvailable) return _sorted(localItems);

    lastLocalCount = localItems.length;
    lastFirestoreCount = -1;
    lastFirestoreError = null;

    debugPrint('[FiltreCTA] list(): isInit=${FirebaseBootstrap.isInitialized} '
        'isOnline=${FirebaseBootstrap.isOnline} '
        'localCache=${localItems.length} docs');

    try {
      final snap = await _col.get();
      final cloudItems = snap.docs
          .map((d) => DevizFiltreCta.fromMap({...d.data(), 'id': d.id}))
          .toList();

      lastFirestoreCount = cloudItems.length;
      lastFirestoreError = null;
      debugPrint('[FiltreCTA] Firestore returned ${cloudItems.length} docs');

      final cloudIds = cloudItems.map((r) => r.id).toSet();
      // Numerele devizelor din cloud (CTA-YYYY-NNNN) — pentru deduplicare
      final cloudNumere =
          cloudItems.map((r) => r.numar).where((n) => n.isNotEmpty).toSet();

      final localOnly = localItems.where((r) {
        // Exclude items deja sincronizate: ID în cloud SAU same numar în cloud
        // (cazul când local- a fost promovat la real ID — evită duplicate)
        if (cloudIds.contains(r.id)) return false;
        if (r.id.startsWith('local-') &&
            r.numar.isNotEmpty &&
            cloudNumere.contains(r.numar)) {
          return false; // deja există în cloud cu ID real — nu mai adăuga local-
        }
        return true;
      }).toList();

      for (final r in localOnly) {
        await OfflineSyncRuntime.instance.queueFiltreCtaUpsert(r.toMap());
      }

      final merged = [...cloudItems, ...localOnly];
      await _saveLocal(merged);
      return _sorted(merged);
    } catch (e) {
      lastFirestoreCount = -1;
      lastFirestoreError = e.toString();
      debugPrint('[FiltreCTA] Eroare Firestore list(): $e');
      return _sorted(localItems);
    }
  }

  List<DevizFiltreCta> _sorted(List<DevizFiltreCta> items) {
    final copy = [...items];
    copy.sort((a, b) => b.updatedAt.compareTo(a.updatedAt));
    return copy;
  }

  // ── Save (upsert) ────────────────────────────────────────────────────────

  Future<DevizFiltreCta> save(DevizFiltreCta record) async {
    DevizFiltreCta toSave;

    if (record.id.isEmpty || record.id.startsWith('local-')) {
      if (_isCloudAvailable) {
        try {
          final map = record.toMap()..remove('id');
          final ref = await _col.add(map);
          toSave =
              DevizFiltreCta.fromMap({...record.toMap(), 'id': ref.id});
        } catch (e) {
          debugPrint('[FiltreCTA] Eroare add Firestore: $e');
          toSave = DevizFiltreCta.fromMap({
            ...record.toMap(),
            'id': 'local-${DateTime.now().millisecondsSinceEpoch}',
          });
        }
      } else {
        toSave = DevizFiltreCta.fromMap({
          ...record.toMap(),
          'id': 'local-${DateTime.now().millisecondsSinceEpoch}',
        });
      }
    } else {
      toSave = record;
    }

    // 1. Salvare locală
    // Dacă ID-ul s-a schimbat (local- → real), elimină intrarea veche din cache
    if (record.id != toSave.id && record.id.startsWith('local-')) {
      final locals = await listLocal();
      await _saveLocal(locals.where((r) => r.id != record.id).toList());
    }
    await _updateLocalCache(toSave);

    // 2. Queue OBLIGATORIU
    await OfflineSyncRuntime.instance.queueFiltreCtaUpsert(toSave.toMap());

    // 3. Firebase direct (best-effort, fire-and-forget — nu blochează UI)
    // Queue-ul de mai sus garantează sync-ul; aceasta e doar optimizare de latență.
    if (_isCloudAvailable && !toSave.id.startsWith('local-')) {
      final map = toSave.toMap()..remove('id');
      _col.doc(toSave.id).set(map, SetOptions(merge: true)).catchError((_) {});
    }

    return toSave;
  }

  // ── Delete ───────────────────────────────────────────────────────────────

  Future<void> delete(String id) async {
    // 1. Ștergere locală
    final locals = await listLocal();
    await _saveLocal(locals.where((r) => r.id != id).toList());

    // 2. Queue delete OBLIGATORIU
    await OfflineSyncRuntime.instance.queueFiltreCtaDelete(id);

    // 3. Firebase direct (best-effort, fire-and-forget — nu blochează UI)
    if (_isCloudAvailable && !id.startsWith('local-')) {
      _col.doc(id).delete().catchError((_) {});
    }
  }

  // ── Sync forțat: publică toate documentele locale în Firestore ───────────

  Future<int> forceSyncLocalToCloud() async {
    if (!_isCloudAvailable) return 0;
    final locals = await listLocal();
    if (locals.isEmpty) return 0;

    // Lucrăm pe o copie mutabilă — înlocuim local- IDs cu cele reale in-place
    final result = List<DevizFiltreCta>.from(locals);
    int synced = 0;

    for (int i = 0; i < result.length; i++) {
      final r = result[i];
      try {
        if (r.id.startsWith('local-') || r.id.isEmpty) {
          final map = r.toMap()..remove('id');
          final ref = await _col.add(map);
          // Înlocuiește in-place — elimină local- ID din listă
          result[i] = DevizFiltreCta.fromMap({...r.toMap(), 'id': ref.id});
          await OfflineSyncRuntime.instance
              .queueFiltreCtaUpsert(result[i].toMap());
        } else {
          final map = r.toMap()..remove('id');
          await _col.doc(r.id).set(map, SetOptions(merge: true));
          await OfflineSyncRuntime.instance.queueFiltreCtaUpsert(r.toMap());
        }
        synced++;
      } catch (e) {
        debugPrint('[FiltreCTA] ❌ forceSyncLocalToCloud error for ${r.id}: $e');
      }
    }

    // Salvează lista actualizată (ID-uri reale în locul celor local-)
    // Deduplicare după ID — elimină eventualele duplicate rămase din sync-uri anterioare
    final seen = <String>{};
    final deduplicated = result.where((r) => seen.add(r.id)).toList();
    await _saveLocal(deduplicated);
    return synced;
  }

  // ── Deduplicare Firestore ────────────────────────────────────────────────
  /// Găsește documentele cu același `numar` în Firestore și șterge duplicatele,
  /// păstrând cel mai recent (după `updated_at`).
  /// Returnează numărul de duplicate șterse.
  Future<int> deduplicateFirestore() async {
    if (!_isCloudAvailable) return 0;

    final snap = await _col.get();
    if (snap.docs.isEmpty) return 0;

    // Grupează documentele după câmpul `numar`
    final Map<String, List<QueryDocumentSnapshot<Map<String, dynamic>>>> byNumar = {};
    for (final doc in snap.docs) {
      final numar = (doc.data()['numar'] as String?) ?? '';
      if (numar.isEmpty) continue;
      byNumar.putIfAbsent(numar, () => []).add(doc);
    }

    int deleted = 0;
    for (final group in byNumar.values) {
      if (group.length <= 1) continue;

      // Sortează descendent după updated_at — primul = cel mai recent
      group.sort((a, b) {
        final aRaw = a.data()['updated_at'] as String?;
        final bRaw = b.data()['updated_at'] as String?;
        final aMs = aRaw != null ? DateTime.tryParse(aRaw)?.millisecondsSinceEpoch ?? 0 : 0;
        final bMs = bRaw != null ? DateTime.tryParse(bRaw)?.millisecondsSinceEpoch ?? 0 : 0;
        return bMs.compareTo(aMs);
      });

      // Păstrează primul (cel mai recent), șterge restul
      for (int i = 1; i < group.length; i++) {
        await _col.doc(group[i].id).delete();
        debugPrint('[FiltreCTA] 🗑️ Duplicat șters: ${group[i].id} (numar=${group.first.data()['numar']})');
        deleted++;
      }
    }

    // Curăță și cache-ul local cu datele curate din Firestore
    if (deleted > 0) {
      await list();
    }
    return deleted;
  }

  // ── Numerotare automată ──────────────────────────────────────────────────

  Future<String> nextNumber() async {
    final year = DateTime.now().year;
    const prefix = 'CTA';
    final prefixYear = '$prefix-$year-';
    final prefixYearNext = '$prefix-${year + 1}-';

    try {
      final localItems = await listLocal();
      final localNums = localItems
          .where((r) => r.numar.startsWith(prefixYear))
          .map((r) {
            final parts = r.numar.split('-');
            return parts.length >= 3 ? int.tryParse(parts.last) ?? 0 : 0;
          })
          .toList();

      if (_isCloudAvailable) {
        try {
          final snap = await _col
              .where('numar', isGreaterThanOrEqualTo: prefixYear)
              .where('numar', isLessThan: prefixYearNext)
              .get();
          final cloudNums = snap.docs.map((d) {
            final n = (d.data()['numar'] ?? '').toString();
            final parts = n.split('-');
            return parts.length >= 3 ? int.tryParse(parts.last) ?? 0 : 0;
          }).toList();
          localNums.addAll(cloudNums);
        } catch (e) {
          debugPrint('[FiltreCtaRepo] citire numere cloud eșuată, folosesc local: $e');
        }
      }

      final maxNum =
          localNums.isEmpty ? 0 : localNums.reduce((a, b) => a > b ? a : b);
      final next = (maxNum + 1).toString().padLeft(4, '0');
      return '$prefixYear$next';
    } catch (_) {
      return '${prefixYear}0001';
    }
  }
}
