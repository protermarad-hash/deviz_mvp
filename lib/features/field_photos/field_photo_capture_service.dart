...
class FieldPhotoCaptureService {
  ...
  static Future<({FieldPhotoRecord record, String? uploadError})>
    ...
    try {
      // Refresh token înainte de upload — previne erori 'unauthorized' la token expirat (>1h idle).
      final authUser = FirebaseAuth.instance.currentUser;
      if (authUser != null) {
        await authUser.getIdToken(true);
      }
      debugPrint('[FieldPhotoUpload] path=field_photos/$normalizedModule/$normalizedEntityId/$cloudFileName');
      debugPrint('[FieldPhotoUpload] bucket=${FirebaseStorage.instance.bucket}');
      debugPrint('[FieldPhotoUpload] user=${authUser?.uid ?? 'null (neautentificat)'}');
      final ref = FirebaseStorage.instance
          ...
          .child(cloudFileName);
      await ref.putFile(
        File(pickedFile.filePath),
        SettableMetadata(
          contentType: _contentTypeFromPath(pickedFile.fileName),
          customMetadata: <String, String>{
            'source_module': normalizedModule,
            'source_entity_id': normalizedEntityId,
          },
        ),
      );
      ...
      debugPrint('[FieldPhotoUpload] ❌ eroare: $e');
      debugPrint('[FieldPhotoUpload] user=${FirebaseAuth.instance.currentUser?.uid ?? 'null'}');
      debugPrint('[FieldPhotoUpload] bucket=${FirebaseStorage.instance.bucket}');
  ...
  static Future<FieldPhotoRecord> retryUpload(FieldPhotoRecord photo) async {
    ...
    // Refresh token înainte — previne unauthorized la token expirat (>1h idle).
    try {
      final authUser = FirebaseAuth.instance.currentUser;
    ...
    try {
      snapshot = await ref.putFile(
        File(localPath),
        SettableMetadata(
          contentType: _contentTypeFromPath(photo.fileName),
          customMetadata: <String, String>{
            'source_module': photo.sourceModule,
            'source_entity_id': photo.sourceEntityId,
          },
        ),
      );
    } catch (e) {
      final errStr = e.toString();
      debugPrint('[FieldPhotoRetry] ❌ upload error: $e');
      debugPrint('[FieldPhotoRetry] user=${FirebaseAuth.instance.currentUser?.uid ?? 'null'}');
      throw Exception(
          errStr.contains('unauthorized')
              ? 'Nu ai permisiunea să încarci fișiere în cloud. '
                  'Bucket: ${FirebaseStorage.instance.bucket}. '
                  'User: ${FirebaseAuth.instance.currentUser?.uid ?? "neautentificat"}. '
                  'Verifică regulile Storage și autentificarea în consolă Firebase.'
