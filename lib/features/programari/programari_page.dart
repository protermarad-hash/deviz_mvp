...
import '../partner_financial/partner_financial_models.dart';
import '../partner_financial/partner_financial_repository.dart';
...
import 'programari_consum_materiale_page.dart';
...
class _ProgramariPageState extends State<ProgramariPage> {
  ...
  // Persistența filtrelor active — se restaurează la redeschiderea modulului
  static const String _filterTeamPreferenceKey =
      'programari_filter_team_v1';
  static const String _filterStatusPreferenceKey =
      'programari_filter_status_v1';
  static const String _filterJobPreferenceKey =
  ...
  bool _loading = true;
  bool _loadAllHistory = false;
  List<ProductCatalogRecord>? _cachedCatalogProducts;
  ...
  Map<String, String> _teamNameById = const {};
  Map<String, Color> _teamColorById = const {};
  // Map O(1) pentru ClientRecord și JobRecord complet (evită for-loop în itemBuilder)
  Map<String, ClientRecord> _clientRecordByIdMap = const {};
  Map<String, JobRecord> _jobRecordByIdMap = const {};
  ...
  Future<void>? _secondaryDataLoadFuture;
  bool _secondaryDataLoading = false;
  bool _secondaryDataReady = false;
  ...
  void _programariLog(String message) {
    debugPrint('[Programari] $message');
  }

  void _setStateLogged(String reason, VoidCallback fn) {
    if (!mounted) return;
    _programariLog('setState reason=$reason');
    setState(fn);
  }

  int get _activeBackgroundTasksCount =>
      _activeBackgroundTasks.length +
      (OfflineSyncRuntime.instance.isSyncing ? 1 : 0);

  void _logActiveBackgroundTasks() {
    _programariLog('active background tasks count=$_activeBackgroundTasksCount');
  }

  void _startBackgroundTask(String label, Future<void> Function() task) {
    final token = '$label#${++_backgroundTaskSerial}';
    final stopwatch = Stopwatch()..start();
    _activeBackgroundTasks[token] = stopwatch;
    _programariLog('$label start');
    _logActiveBackgroundTasks();
    task().then((_) {
      _programariLog('$label end duration_ms=${stopwatch.elapsedMilliseconds}');
    }).catchError((error, stackTrace) {
      _programariLog('$label error duration_ms=${stopwatch.elapsedMilliseconds} error=$error');
      FirebaseBootstrap.registerRuntimeError(error);
    }).whenComplete(() {
      _activeBackgroundTasks.remove(token);
      _logActiveBackgroundTasks();
    });
  }
  ...
  void initState() {
    ...
    // Reîncarcă clienții doar debounced, ca să evităm storm-ul de reload-uri.
  ...
  void dispose() {
    ...
    LocalAppDataRepository.clientsChangeCount.removeListener(
      _onClientsChangeCountUpdated,
    );
  ...
  /// Reîncarcă doar lista de clienți fără a reîncărca toate datele.
  void _onClientsChangeCountUpdated() {
    if (_secondaryDataLoading) {
      _programariLog('clientsChangeCount skip reason=secondary_loading');
      return;
    }
    _clientsReloadDebounce?.cancel();
    _programariLog('clientsChangeCount debounce scheduled');
    _clientsReloadDebounce = Timer(const Duration(milliseconds: 250), () {
      unawaited(_reloadClientsOnly(reason: 'clientsChangeCount'));
    });
  }

  Future<void> _reloadClientsOnly({String reason = 'manual'}) async {
    if (_secondaryDataLoading) {
      _programariLog('reload clients skip reason=secondary_loading');
      return;
    }
    if (_clientsReloadInProgress) {
      _programariLog('reload clients skip reason=already_running');
      return;
    }
    final now = DateTime.now();
    if (_lastClientsReloadAt != null &&
        now.difference(_lastClientsReloadAt!) <
            const Duration(milliseconds: 250)) {
      _programariLog('reload clients skip reason=debounced');
      return;
    }
    _clientsReloadInProgress = true;
    _lastClientsReloadAt = now;
    _programariLog('reload clients start source=$reason');
    try {
      ...
      if (!mounted) return;
      _setStateLogged('reload clients only', () {
        _clients = results[0] as List<LookupItem>;
        _clientRecords = results[1] as List<ClientRecord>;
        _clientNameById = {
      ...
      FirebaseBootstrap.registerRuntimeError(error);
      _programariLog('reload clients error source=$reason error=$error');
    } finally {
      _clientsReloadInProgress = false;
      _programariLog('reload clients end source=$reason');
    }
  }

  Future<void> _load() async {
    // syncPending() NU se apelează aici — rulează deja la startup, la fiecare 30s
    // din bootstrap și la resume. A-l apela și din _load() e redundant și costisitor.
    _refreshCloudRepositories();
    final stopwatch = Stopwatch()..start();
    try {
      // ── Faza 1: programări + utilizator curent → afișăm lista imediat ──
      final phase1 = await Future.wait<dynamic>(<Future<dynamic>>[
        _listAppointmentsResolved(),
        _loadCurrentAuthUserResolved(),
      ]);
      if (!mounted) return;
      _setStateLogged('load phase1 results', () {
        _items = phase1[0] as List<Appointment>;
        _currentAuthUser = phase1[1] as FieldAuthUser?;
        _loading = false;
        _updateFilteredCache();
      });
      _startBackgroundTask('photo counts background', _loadPhotoCountsAsync);
      await _handleInitialNavigationIntents();

      // ── Faza 2: date secundare în background (dropdown-uri, filtre, echipe) ──
      _startBackgroundTask('secondary preload', _loadSecondaryDataAsync);
    } catch (error) {
      FirebaseBootstrap.registerRuntimeError(error);
      if (mounted) {
        _cloudFallbackReason = _shortCloudError(error);
        _setStateLogged('load error', () => _loading = false);
      }
    } finally {
      _programariLog(
        'load end duration_ms=${stopwatch.elapsedMilliseconds} count=${_items.length}',
      );
    }
  }

  // Date secundare: jobs, clienți, echipe, angajați, parteneri, kit-uri.
  // Apelat în background după ce lista de programări este deja vizibilă.
  Future<void> _loadSecondaryDataAsync() {
    final active = _secondaryDataLoadFuture;
    if (active != null) {
      _programariLog('secondary preload skip reason=already_running');
      return active;
    }
    final future = _loadSecondaryDataAsyncImpl();
    _secondaryDataLoadFuture = future;
    return future.whenComplete(() {
      if (identical(_secondaryDataLoadFuture, future)) {
        _secondaryDataLoadFuture = null;
      }
    });
  }

  Future<void> _loadSecondaryDataAsyncImpl() async {
    _secondaryDataLoading = true;
    try {
      final results = await Future.wait<dynamic>(<Future<dynamic>>[
        _listJobsLookupResolved(),
        _listClientsLookupResolved(),
        _listClientRecordsResolved(),
        _listJobRecordsResolved(),
      ...
      if (!mounted) return;
      final fetchedTeams = results[4] as List<Team>;
      ...
      _setStateLogged('load secondary data', () {
        ...
        _clientRecords = results[2] as List<ClientRecord>;
        _jobRecords = results[3] as List<JobRecord>;
        _teams = fetchedTeams;
        _masterEmployees = results[5] as List<MasterEmployee>;
        _masterTeams = masterTeamsResolved;
        _partnerRecords = results[6] as List<PartnerRecord>;
        _materialKitTemplates =
            results[7] as List<AppointmentMaterialKitTemplate>;

        // Construieste Maps O(1) pentru lookup rapid în build()
        // Evita bucle O(n) per card (ex: 200 progr. × 100 clienti = 20k iter.)
        _clientNameById = {
          for (final c in _clients)
            if (c.id.trim().isNotEmpty)
              c.id: c.name.trim().isEmpty ? c.id : c.name,
        ...
        _jobLabelById = {
          for (final j in _jobs)
            if (j.id.trim().isNotEmpty)
              j.id: j.name.trim().isEmpty ? j.id : j.name,
        };
        _teamNameById = {
          for (final t in fetchedTeams)
            if (t.id.trim().isNotEmpty)
              t.id: t.name.trim().isEmpty ? t.id : t.name,
          for (final t in masterTeamsResolved)
            if (t.id.trim().isNotEmpty)
              t.id: t.name.trim().isEmpty ? t.id : t.name,
        };
        _teamColorById = {
          for (final t in masterTeamsResolved)
            if (t.id.trim().isNotEmpty && t.colorValue != 0)
              t.id: Color(t.colorValue),
        };
        // Maps O(1) pentru record complet — elimină for-loop din itemBuilder
        _clientRecordByIdMap = {
          for (final c in results[2] as List<ClientRecord>)
            if (c.id.trim().isNotEmpty) c.id: c,
        };
        _jobRecordByIdMap = {
          for (final j in results[3] as List<JobRecord>)
            if (j.id.trim().isNotEmpty) j.id: j,
        };

        _updateFilteredCache();
      });
      _secondaryDataReady = true;
      _programariLog(
        'secondary data counts clients=${_clientRecords.length} jobs=${_jobRecords.length} partners=${_partnerRecords.length}',
      );
    } catch (error) {
      FirebaseBootstrap.registerRuntimeError(error);
    } finally {
      _secondaryDataLoading = false;
    }
  }

  // Cache catalog produse — evită reîncărcarea la fiecare deschidere dialog.
  Future<List<ProductCatalogRecord>> _getCatalogProducts() async {
    if (_cachedCatalogProducts != null) return _cachedCatalogProducts!;
    try {
      final all = await _productCatalogService.listProducts();
      _cachedCatalogProducts = all
          .where((p) => p.isActive && p.name.trim().isNotEmpty)
          .toList(growable: false);
    } catch (_) {
      _cachedCatalogProducts = const [];
    }
    return _cachedCatalogProducts!;
  }

  Future<void> _loadPhotoCountsAsync() async {
    // OPTIMIZARE CRITICĂ: folosim NUMAI cache-ul local pentru numărare.
    // Varianta cu listFieldPhotos(sourceModule:'programari') descarca TOATE pozele
    // din Firestore (sute de documente) doar pentru a le număra — extrem de lent.
    // Cache-ul local conține pozele dispozitivului curent și e suficient pentru badge-uri.
    try {
      final photos = await widget.repository.listFieldPhotosLocalOnly(
        sourceModule: 'programari',
      );
      final counts = <String, int>{};
      for (final photo in photos) {
        final id = photo.sourceEntityId.trim();
        if (id.isNotEmpty) {
  ...
  bool get _hasEditorReferenceData =>
      _secondaryDataReady ||
      _clientRecords.isNotEmpty ||
      _jobRecords.isNotEmpty ||
      _partnerRecords.isNotEmpty;

  Future<void> _ensureSecondaryDataReadyForEditor() async {
    if (_hasEditorReferenceData) {
      return;
    }
    final stopwatch = Stopwatch()..start();
    _programariLog('editor preload waiting for secondary data');
    final future = _secondaryDataLoadFuture ?? _loadSecondaryDataAsync();
    var loadingShown = false;
    unawaited(Future<void>.delayed(const Duration(milliseconds: 150)).then((_) {
      if (!mounted || _hasEditorReferenceData || loadingShown) {
        return Future<void>.value();
      }
      loadingShown = true;
      _programariLog('editor preload showing loading state');
        ...
        builder: (dialogContext) => const AlertDialog(
          content: Row(
            children: [
              ...
              SizedBox(width: 12),
              Expanded(
                child: Text('Se încarcă clienții, lucrările și partenerii...'),
              ),
            ],
          ),
        ),
    ...
    _programariLog(
      'editor preload wait end duration_ms=${stopwatch.elapsedMilliseconds} ready=$_secondaryDataReady',
    );
  }
  ...
  Future<void> _loadAllPreferences() async {
    ...
    // Filtre active — restaurate la redeschiderea modulului
    final savedFilterTeam =
        (prefs.getString(_filterTeamPreferenceKey) ?? '').trim();
    final savedFilterStatus =
        (prefs.getString(_filterStatusPreferenceKey) ?? '').trim();
    final savedFilterJob =
    ...
    _setStateLogged('load preferences', () {
      ...
      // Restaurăm filtrele active
      _teamFilter = savedFilterTeam;
      _statusFilter = savedFilterStatus;
      _jobFilter = savedFilterJob;
      if (savedFilterScope.isNotEmpty) {
        _employeeScopeMode = _EmployeeScopeMode.values.firstWhere(
          (e) => e.name == savedFilterScope,
  ...
  /// Salvează filtrele active în SharedPreferences — apelat la fiecare
  /// schimbare de filtru ca utilizatorul să regăsească selecția la revenire.
  Future<void> _persistFilterPreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await Future.wait([
      prefs.setString(_filterTeamPreferenceKey, _teamFilter),
      prefs.setString(_filterStatusPreferenceKey, _statusFilter),
      prefs.setString(_filterJobPreferenceKey, _jobFilter),
      prefs.setString(
  ...
  Color? _teamColorForAppointment(Appointment item) {
    // O(n) în loc de O(n²) — _teamColorById e Map pre-construit
    for (final teamId in _appointmentTeamIds(item)) {
      final color = _teamColorById[teamId];
  ...
  Future<List<Appointment>> _listAppointmentsResolved() async {
    final items = _loadAllHistory
        ? await widget.repository.listAllAppointments()
        : await widget.repository.listAppointments();
  ...
  Future<List<ClientRecord>> _listClientRecordsResolved() async {
    final cloud = _clientiCloudRepository;
    if (cloud == null) {
      return widget.repository.listClients();
    }
    try {
      return await cloud.listClients();
    } catch (error) {
      FirebaseBootstrap.registerRuntimeError(error);
      _cloudFallbackReason = _shortCloudError(error);
      return widget.repository.listClients();
  ...
  Future<List<JobRecord>> _listJobRecordsResolved() async {
    final cloud = _lucrariCloudRepository;
    if (cloud == null) {
      return widget.repository.listJobs();
    }
    try {
      return await cloud.listJobs();
    } catch (error) {
      FirebaseBootstrap.registerRuntimeError(error);
      _cloudFallbackReason = _shortCloudError(error);
      return widget.repository.listJobs();
  ...
  Future<void> _reloadAppointmentsOnly() async {
    try {
      ...
      if (!mounted) return;
      _setStateLogged('reload appointments only', () {
        _items = items;
        _updateFilteredCache();
      });
    } catch (error) {
  ...
  /// Returnează true dacă s-au schimbat câmpuri relevante pentru notificare email.
  /// Email NU se trimite pentru: plăți, încasări, materiale, prețuri, poze, note interne.
  bool _hasRelevantChangesForEmail(Appointment current, Appointment previous) {
    if (current.title.trim() != previous.title.trim()) { return true; }
    if (current.clientId.trim() != previous.clientId.trim()) { return true; }
    if (current.clientName.trim() != previous.clientName.trim()) { return true; }
    if (_normalizeStatusValue(current.status) !=
        _normalizeStatusValue(previous.status)) {
      return true;
    }
    if (current.teamId.trim() != previous.teamId.trim()) { return true; }
    if (current.assignedUserId.trim() != previous.assignedUserId.trim()) {
      return true;
    }
    if (current.assignedUserEmail.trim() !=
        previous.assignedUserEmail.trim()) {
      return true;
    }
    final currentTeams = (current.assignedTeamIds..sort()).join(',');
    final previousTeams = (previous.assignedTeamIds..sort()).join(',');
    if (currentTeams != previousTeams) { return true; }
    final currentEmployees = (current.assignedEmployeeIds..sort()).join(',');
    final previousEmployees = (previous.assignedEmployeeIds..sort()).join(',');
    if (currentEmployees != previousEmployees) { return true; }
    if (current.location.trim() != previous.location.trim()) { return true; }
    if (current.type.trim() != previous.type.trim()) { return true; }
    return false;
  }
  ...
  Future<void> _notifyAppointmentSaved({
    ...
    // Pentru appointmentUpdated, verificăm dacă s-au schimbat câmpuri relevante.
    // Nu trimitem email pentru modificări financiare, materiale, note interne sau poze.
    if (eventType == NotificationEventType.appointmentUpdated &&
        previous != null &&
        !_hasRelevantChangesForEmail(item, previous)) {
      return;
    }
  ...
  /// Sincronizează automat tranzacțiile financiare ale partenerilor
  /// din câmpurile financiare ale programării:
  ///   - forPartnerInvoiceAmount → PartnerTransaction (intrare, incasare_programare)
  ///   - executingPartnerCommission → PartnerTransaction (iesire, plata_programare)
  /// ID-urile sunt deterministe (ptxn_{id}_for / ptxn_{id}_exec) pentru upsert corect.
  Future<void> _syncPartnerFinancialFromAppointment(
    Appointment item, {
    Appointment? previous,
  }) async {
    ...
    // --- 1. forPartner: partenerul pentru care lucrăm (noi încasăm de la el) ---
    final forTxnId = 'ptxn_${item.id}_for';

    // Dacă partenerul s-a schimbat față de versiunea anterioară → șterge vechea tranzacție
    final prevForPartnerId = previous?.forPartnerId.trim() ?? '';
    final currForPartnerId = item.forPartnerId.trim();
    if (prevForPartnerId.isNotEmpty &&
        prevForPartnerId != currForPartnerId) {
      await repo.deleteTransaction(forTxnId);
      // Queue apelat din PartnerFinancialRepository.deleteTransaction() (CLAUDE.md)
    }

    if (currForPartnerId.isNotEmpty && item.forPartnerInvoiceAmount > 0) {
      final forTxn = PartnerTransaction(
        id: forTxnId,
        partnerId: currForPartnerId,
        partnerName: item.forPartnerName.trim(),
        type: PartnerTransactionType.incasareProgramare,
        direction: PartnerTransactionDirection.intrare,
        amount: item.forPartnerInvoiceAmount,
        date: item.scheduledDate,
        description: 'Programare: $appointmentLabel',
        referenceId: item.id,
        referenceType: 'programare',
        paymentMethod: PartnerTransactionPaymentMethod.transfer,
        status: _mapPartnerPaymentStatus(item.forPartnerReceiveStatus),
        createdAt: now,
        updatedAt: now,
      );
      await repo.upsertTransaction(forTxn);
      // Queue apelat din PartnerFinancialRepository.upsertTransaction() (CLAUDE.md)
    } else if (currForPartnerId.isEmpty || item.forPartnerInvoiceAmount <= 0) {
      // Suma eliminată sau partenerul eliminat → șterge tranzacția dacă există
      await repo.deleteTransaction(forTxnId);
      // Queue apelat din PartnerFinancialRepository.deleteTransaction() (CLAUDE.md)
    }

    // --- 2. executingPartner: partenerul care execută (noi îi plătim) ---
    final execTxnId = 'ptxn_${item.id}_exec';

    final prevExecPartnerId = previous?.executingPartnerId.trim() ?? '';
    final currExecPartnerId = item.executingPartnerId.trim();
    if (prevExecPartnerId.isNotEmpty &&
        prevExecPartnerId != currExecPartnerId) {
      await repo.deleteTransaction(execTxnId);
      // Queue apelat din PartnerFinancialRepository.deleteTransaction() (CLAUDE.md)
    }

    if (currExecPartnerId.isNotEmpty && item.executingPartnerCommission > 0) {
      final execTxn = PartnerTransaction(
        id: execTxnId,
        partnerId: currExecPartnerId,
        partnerName: item.executingPartnerName.trim(),
        type: PartnerTransactionType.plataProgramare,
        direction: PartnerTransactionDirection.iesire,
        amount: item.executingPartnerCommission,
        date: item.scheduledDate,
        description: 'Programare: $appointmentLabel',
        referenceId: item.id,
        referenceType: 'programare',
        paymentMethod: PartnerTransactionPaymentMethod.transfer,
        status: _mapPartnerPaymentStatus(item.executingPartnerPaymentStatus),
        createdAt: now,
        updatedAt: now,
      );
      await repo.upsertTransaction(execTxn);
      // Queue apelat din PartnerFinancialRepository.upsertTransaction() (CLAUDE.md)
    } else if (currExecPartnerId.isEmpty ||
        item.executingPartnerCommission <= 0) {
      await repo.deleteTransaction(execTxnId);
      // Queue apelat din PartnerFinancialRepository.deleteTransaction() (CLAUDE.md)
    }
  }

  /// Convertește PartnerPaymentStatus (din programare) în PartnerTransactionStatus (financiar).
  PartnerTransactionStatus _mapPartnerPaymentStatus(
    ...
    switch (status) {
      ...
      case PartnerPaymentStatus.platitPartial:
        return PartnerTransactionStatus.partial;
  ...
  Future<void> _confirmAndDeleteAppointment(Appointment item) async {
    ...
    if (!mounted) return;
    // Optimistic UI: scoate din listă imediat — nu așteaptă Firestore
    _setStateLogged('delete optimistic ui', () {
      _items = _items.where((a) => a.id != item.id).toList(growable: false);
      _updateFilteredCache();
    });
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Programarea a fost stearsa.')),
    );
    // Operații async în fundal (nu blochează UI)
    ...
    _deleteAppointmentResolved(item.id).catchError((e) {
      if (mounted) {
        ...
        _reloadAppointmentsOnly(); // restaurează starea la eroare
      }
    });
  }
  ...
  Widget _linkedDocumentsChip(Appointment item) {
    final count = item.linkedDocuments.length;
    final hasDocuments = count > 0;
    return Chip(
      avatar: Icon(
        hasDocuments ? Icons.attach_file : Icons.attach_file_outlined,
        size: 16,
        color: hasDocuments ? Colors.blue : null,
      ),
      label: Text(hasDocuments ? '$count doc.' : 'Fără doc.'),
      visualDensity: VisualDensity.compact,
    );
  }
  ...
  ClientRecord? _clientRecordById(String clientId) {
    ...
    return _clientRecordByIdMap[id];
  }

  // O(1) lookup — înlocuiește for-loop O(n)
  JobRecord? _jobRecordById(String jobId) {
    final id = jobId.trim();
    if (id.isEmpty) return null;
    return _jobRecordByIdMap[id];
  }
  ...
  String _clientName(String clientId) {
    ...
    // O(1) lookup din Map pre-construit în _loadSecondaryDataAsync()
    return _clientNameById[clientId] ?? clientId;
  }
  ...
  String _jobLabel(String jobId) {
    ...
    // O(1) lookup din Map pre-construit în _loadSecondaryDataAsync()
    return _jobLabelById[jobId] ?? jobId;
  }
  ...
  String _teamName(String teamId) {
    ...
    // O(1) lookup din Map pre-construit în _loadSecondaryDataAsync()
    return _teamNameById[teamId] ?? teamId;
  }
  ...
  void _updateFilteredCache() {
    ...
    _cachedFilteredItems = scoped.where((item) {
      ...
      if (_documentFilter == 'cu_documente' &&
          item.linkedDocuments.isEmpty) {
        return false;
      }
      if (_documentFilter == 'fara_documente' &&
          item.linkedDocuments.isNotEmpty) {
        return false;
      }
      return true;
    }).toList(growable: false);
  ...
  Future<void> _openParteneriReport() async {
    await Navigator.of(context, rootNavigator: true).push(
      MaterialPageRoute<void>(
        fullscreenDialog: true,
        builder: (_) => ProgramariParteneriPage(
          appointments: _items,
          onAppointmentSaved: (updated) async {
  ...
  Future<void> _openConsumMaterialeReport() async {
    await Navigator.of(context, rootNavigator: true).push(
      MaterialPageRoute<void>(
        fullscreenDialog: true,
        builder: (_) => ProgramariConsumMaterialePage(
          appointments: _items,
        ),
      ),
    );
  ...
  /// Dialog bon de consum SmartBill pentru o programare cu materiale consumate.
  Future<void> _openBonConsumDialog(
    BuildContext parentContext,
    Appointment item,
  ) async {
    // Încarcă setările SmartBill din profilul firmei
    final profile = await widget.repository.loadCompanyProfile();
    final sbSettings = profile.smartBillSettings;

    if (!mounted) return;

    if (!sbSettings.isConfigured) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'SmartBill nu este configurat. Mergi la Setări firmă → SmartBill.',
          ),
        ),
      );
      return;
    ...
    if (!sbSettings.isConsumptionConfigured) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Completează gestiunea și seria pentru bon de consum în Setări firmă → SmartBill.',
          ),
        ),
      );
      return;
    ...
    final lines = item.materialUsage.lines
        .where((l) => l.name.trim().isNotEmpty && l.quantity > 0)
        .toList();

    if (lines.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Programarea nu are materiale consumate.')),
      );
      return;
    }
    ...
    await showDialog<void>(
      context: context,
      builder: (ctx) => ProgramariBonConsumDialog(
        appointment: item,
        settings: sbSettings,
        lines: lines,
      ),
  ...
  Future<void> _openEmployeeMaterialUsageDialog(Appointment item) async {
    // Identifică liniile "extra" (nu din kit) = ID timestamp-based
    List<AppointmentMaterialUsageLine> extraLinesForKit(String kitId) {
      final template = _findMaterialKitTemplateById(kitId);
      if (template == null) return List.from(item.materialUsage.lines);
      final kitComponentIds = template.components.map((c) => c.id).toSet();
      return item.materialUsage.lines
          .where((l) => !kitComponentIds.contains(l.id))
          .toList();
    }
                      ...
                      if (kitOptions.isNotEmpty) ...[
                        ...
                        DropdownButtonFormField<String>(
                          initialValue: kitOptions.any(
                            (t) => t.id == selectedKitId,
                          )
                              ? selectedKitId
                              : null,
                          decoration: const InputDecoration(
                            labelText: 'Selectează kit',
                          ),
  ...
  Future<void> _quickPostpone(Appointment item) async {
    final reason = await showDialog<String>(
      context: context,
      builder: (_) => const ProgramariPostponeReasonDialog(),
    );
        ...
        children: [
          ...
          TextButton.icon(
            onPressed: () {
              ...
              // Șterge și din SharedPreferences
          ...
          DropdownButtonFormField<String>(
            ...
            items: const <DropdownMenuItem<String>>[
              DropdownMenuItem<String>(
                value: '',
                child: Text('Toate programările'),
              ),
              DropdownMenuItem<String>(
                value: 'cu_documente',
                child: Text('Cu documente atașate'),
              ),
              DropdownMenuItem<String>(
                value: 'fara_documente',
                child: Text('Fără documente atașate'),
              ),
            ],
        ...
        child: Padding(
          ...
          child: Wrap(
            ...
            children: [
              ...
              FilterChip(
                avatar: Icon(
                  _loadAllHistory ? Icons.history : Icons.history_outlined,
                  size: 18,
                ),
                label: Text(_loadAllHistory ? 'Tot istoricul' : 'Ultimele 14 luni'),
                selected: _loadAllHistory,
                onSelected: (value) {
                  setState(() => _loadAllHistory = value);
    ...
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => FieldPhotosPage(
          ...
          onPhotoSaved: (photo) async {
            final uploaderName = _employeeLabelForAppointment(item);
            final appointmentTitle = item.title.trim().isEmpty
                ? 'Programare'
                : item.title.trim();
            await _notificationService.dispatchEvent(
              NotificationDispatchRequest(
                eventType: NotificationEventType.fieldPhotoUploaded,
                title: 'Poze noi — $appointmentTitle',
                message: '$uploaderName a încărcat o poză la programarea "$appointmentTitle".',
                sourceModule: 'programari',
                sourceEntityId: item.id,
                sourceLabel: appointmentTitle,
                recipientRoleKeys: const <String>['admin', 'office', 'birou'],
                metadata: <String, dynamic>{
  ...
  // ── Metode dialog rapid încasare / plată ─────────────────────────────────

  Color _financialStatusColor(AppointmentFinancialStatus status) {
    switch (status) {
  ...
  Color _partnerPaymentColor(PartnerPaymentStatus status) {
    switch (status) {
      case PartnerPaymentStatus.neplatit:
        return Colors.red.shade600;
      case PartnerPaymentStatus.platitPartial:
  ...
  Widget _buildFinancialStatusBadge(Appointment item) {
    if (!_canManageAppointmentFinancials) return const SizedBox.shrink();
    final clientColor = _financialStatusColor(item.adminFinancialStatus);
    final hasPartner = item.executingPartnerId.trim().isNotEmpty;
    final partnerColor = hasPartner
        ? _partnerPaymentColor(item.executingPartnerPaymentStatus)
        : null;

    // Construiește textul cu suma concretă
    String clientText = item.adminFinancialStatus.label;
    if (item.adminCollectedAmount > 0 && item.interventionPrice > 0 &&
        item.adminCollectedAmount < item.interventionPrice) {
      clientText += ' · ${item.adminCollectedAmount.toStringAsFixed(0)}/${item.interventionPrice.toStringAsFixed(0)} ${item.adminCollectedCurrency}';
    } else if (item.adminCollectedAmount > 0) {
      clientText += ' · ${item.adminCollectedAmount.toStringAsFixed(0)} ${item.adminCollectedCurrency}';
    } else if (item.interventionPrice > 0 &&
        item.adminFinancialStatus == AppointmentFinancialStatus.neincasata) {
      clientText += ' · ${item.interventionPrice.toStringAsFixed(0)} ${item.interventionPriceCurrency}';
    }

    String partnerText = item.executingPartnerPaymentStatus.label;
    if (item.executingPartnerCommission > 0) {
      partnerText += ' · ${item.executingPartnerCommission.toStringAsFixed(0)} ${item.executingPartnerCommissionCurrency}';
    }
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: clientColor.withValues(alpha: 0.4)),
          ),
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.payments_outlined, size: 14, color: clientColor),
              const SizedBox(width: 4),
              Text(
                clientText,
                style: TextStyle(fontSize: 12, color: clientColor, fontWeight: FontWeight.w600),
              ),
            ],
          ),
        ),
        if (hasPartner && partnerColor != null)
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(
              color: partnerColor.withValues(alpha: 0.12),
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: partnerColor.withValues(alpha: 0.4)),
            ),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(Icons.send_outlined, size: 14, color: partnerColor),
                const SizedBox(width: 4),
                Text(
                  partnerText,
                  style: TextStyle(fontSize: 12, color: partnerColor, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
  ...
  Future<void> _showQuickCollectionDialog(Appointment item) async {
    if (!mounted) return;
    var selectedStatus = item.adminFinancialStatus;
    var selectedMethod = 'Cash';

    // Suma precompletată: ce e deja înregistrat sau prețul de intervenție
    final amountController = TextEditingController(
      text: item.adminCollectedAmount > 0
          ? item.adminCollectedAmount.toStringAsFixed(2)
          : item.interventionPrice > 0
              ? item.interventionPrice.toStringAsFixed(2)
              : '',
    );
    final notesController = TextEditingController(text: item.adminFinancialNotes);
    var selectedDate = item.adminDueDate ?? DateTime.now();

    // Informații context
    final clientName = item.clientName.trim().isNotEmpty
        ? item.clientName.trim()
        : (_clientRecordById(item.clientId)?.name.trim() ?? '');
    final lucrare = item.title.trim();
    final deIncasat = item.interventionPrice;
    final dejaIncasat = item.adminCollectedAmount;
    final ramane = (deIncasat - dejaIncasat).clamp(0.0, double.infinity);
    final currency = item.interventionPriceCurrency;

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogCtx) => StatefulBuilder(
        builder: (_, setDialogState) => AlertDialog(
          title: const Text('Înregistrare încasare'),
          content: SizedBox(
            width: 440,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(
                        color: Theme.of(dialogCtx).colorScheme.primary.withValues(alpha: 0.25),
                      ),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        if (lucrare.isNotEmpty) ...[
                          Row(
                            children: [
                              const Icon(Icons.build_outlined, size: 16),
                              const SizedBox(width: 6),
                              Expanded(child: Text('Lucrare: $lucrare')),
                            ],
                          ),
                          const SizedBox(height: 8),
                        ],
                        ...
                        if (deIncasat > 0)
                          _contextRow(
                            ...
                            value: '${deIncasat.toStringAsFixed(2)} $currency',
                            bold: true,
                          ),
                        if (dejaIncasat > 0) ...[
                          const SizedBox(height: 4),
                          _contextRow(
                            icon: Icons.check_circle_outline,
                            label: 'Deja înregistrat',
                            value: '${dejaIncasat.toStringAsFixed(2)} $currency',
                            color: Colors.green.shade700,
                          ),
                          if (deIncasat > 0 && ramane > 0) ...[
                            const SizedBox(height: 4),
                            _contextRow(
                              icon: Icons.hourglass_bottom_outlined,
                              label: 'Rămâne de încasat',
                              value: '${ramane.toStringAsFixed(2)} $currency',
                        ...
                        _contextRow(
                          icon: Icons.info_outline,
                          label: 'Status curent',
                          value: item.adminFinancialStatus.label,
                        ),
                      ],
                    ),
                  ),
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Sumă încasată acum',
                      suffixText: 'RON',
                      helperText: 'Completează suma efectiv primită',
                    ),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                  ...
                  DropdownButtonFormField<AppointmentFinancialStatus>(
                    initialValue: selectedStatus,
                    decoration: const InputDecoration(labelText: 'Status financiar după înregistrare'),
                    items: AppointmentFinancialStatus.values
    ...
    final amount =
        double.tryParse(amountController.text.trim().replaceAll(',', '.')) ??
            item.adminCollectedAmount;
    final notes = notesController.text.trim();
    final updated = item.copyWith(
      adminCollectedAmount: amount,
      adminFinancialStatus: selectedStatus,
      adminDueDate: selectedDate,
      adminFinancialNotes: notes.isNotEmpty ? notes : item.adminFinancialNotes,
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Încasare de la $clientName: ${amount.toStringAsFixed(2)} RON — $selectedMethod',
          ),
        ),
      );
    }
  ...
  Future<void> _showQuickPartnerPaymentDialog(Appointment item) async {
    ...
    final amountController = TextEditingController(
      text: item.executingPartnerCommission > 0
          ? item.executingPartnerCommission.toStringAsFixed(2)
          : '',
    );
    final notesController = TextEditingController(text: item.executingPartnerPaymentNotes);
    var selectedDate = item.executingPartnerPaymentDate ?? DateTime.now();
    var selectedMethod = 'Transfer';

    // Rezolvă numele partenerului — întâi din appointment, apoi din lista locală
    final partnerName = item.executingPartnerName.trim().isNotEmpty
        ? item.executingPartnerName.trim()
        : () {
            try {
              final rec = _partnerRecords.firstWhere(
                (p) => p.id == item.executingPartnerId,
                orElse: () => throw StateError(''),
              );
              return rec.name.trim().isNotEmpty ? rec.name : 'Partener executant';
            } catch (_) {
              return 'Partener executant';
            }
          }();
    ...
    final clientName = item.clientName.trim().isNotEmpty
        ...
        : (_clientRecordById(item.clientId)?.name.trim() ?? '');
    final lucrare = item.title.trim();
    final comision = item.executingPartnerCommission;
    final commCurrency = item.executingPartnerCommissionCurrency;

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogCtx) => StatefulBuilder(
        builder: (_, setDialogState) => AlertDialog(
          title: const Text('Plată partener executant'),
          content: SizedBox(
            width: 440,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                      ...
                      children: [
                        ...
                        if (clientName.isNotEmpty) ...[
                          ...
                          Row(
                            children: [
                              const Icon(Icons.person_outline, size: 16),
                              const SizedBox(width: 6),
                              Expanded(child: Text('Client final: $clientName')),
                            ],
                          ),
                        ],
                        const SizedBox(height: 8),
                        ...
                        if (comision > 0)
                          _contextRow(
                            ...
                            label: 'Comision datorat',
                            value: '${comision.toStringAsFixed(2)} $commCurrency',
                            bold: true,
                          ),
                        const SizedBox(height: 4),
                        _contextRow(
                          icon: Icons.info_outline,
                          label: 'Status curent',
                          value: item.executingPartnerPaymentStatus.label,
                        ),
                        if (item.executingPartnerPaymentDate != null) ...[
                          const SizedBox(height: 4),
                          _contextRow(
                            icon: Icons.check_circle_outline,
                            label: 'Plătit pe data',
                            value: _formatDate(item.executingPartnerPaymentDate!),
                            color: Colors.green.shade700,
                          ),
                        ],
                      ],
                  ...
                  TextField(
                    ...
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(
                      labelText: 'Sumă plătită',
                      suffixText: 'RON',
                      helperText: 'Suma efectiv transferată partenerului',
                    ),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    initialValue: selectedMethod,
                    decoration: const InputDecoration(labelText: 'Modalitate plată'),
                    items: const [
                      DropdownMenuItem(value: 'Cash', child: Text('Cash')),
                      DropdownMenuItem(value: 'Transfer', child: Text('Transfer bancar')),
                    ],
                    onChanged: (v) => setDialogState(() => selectedMethod = v ?? 'Transfer'),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<PartnerPaymentStatus>(
                    initialValue: selectedStatus,
                    decoration: const InputDecoration(labelText: 'Status plată partener'),
                    items: PartnerPaymentStatus.values
                        .map((s) => DropdownMenuItem(value: s, child: Text(s.label)))
                        .toList(growable: false),
                    onChanged: (v) =>
    ...
    final amount = double.tryParse(
        ...
        item.executingPartnerCommission;
    final notes = notesController.text.trim();
    final updated = item.copyWith(
      executingPartnerPaymentStatus: selectedStatus,
      executingPartnerPaymentDate: selectedDate,
      executingPartnerPaymentNotes: notes.isNotEmpty ? notes : item.executingPartnerPaymentNotes,
    );
    setState(() {
      final idx = _items.indexWhere((e) => e.id == updated.id);
      if (idx >= 0) _items[idx] = updated;
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Plată partener salvată: ${amount.toStringAsFixed(2)} RON')),
      );
    }
  }

  // ── Materiale din catalog (F3) ──────────────────────────────────────────

  // Dialogul de selecție produs — returnează linia nouă sau null
  Future<AppointmentMaterialUsageLine?> _showPickProductFromCatalogDialog(
    List<ProductCatalogRecord> allProducts,
  ) async {
    if (!mounted) return null;
    final quantityCtrl = TextEditingController(text: '1');
    final priceCtrl = TextEditingController();
    var localSearch = '';
    ProductCatalogRecord? selectedProduct;

    List<ProductCatalogRecord> filtered() {
    ...
    final result = await showDialog<AppointmentMaterialUsageLine>(
      context: context,
      builder: (addCtx) => StatefulBuilder(
        builder: (_, setAddState) {
          final filteredList = filtered();
          return AlertDialog(
            title: const Text('Alege material din catalog'),
            content: SizedBox(
              width: 520,
              height: 500,
              child: Column(
                children: [
                  TextField(
                    ...
                    onChanged: (v) => setAddState(() => localSearch = v),
                  ),
                  const SizedBox(height: 8),
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        final prod = filteredList[i];
                        final isSelected = selectedProduct?.id == prod.id;
                        return ListTile(
                          selected: isSelected,
                          title: Text(prod.name),
                          subtitle: Text(
                            '${prod.unit}${prod.listPrice > 0 ? ' — ${prod.listPrice.toStringAsFixed(2)} RON' : ''}${prod.category.isNotEmpty ? ' | ${prod.category}' : ''}',
                          ),
                          onTap: () => setAddState(() {
                            selectedProduct = prod;
                            if (priceCtrl.text.trim().isEmpty && prod.listPrice > 0) {
                              priceCtrl.text = prod.listPrice.toStringAsFixed(2);
                            }
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            keyboardType: const TextInputType.numberWithOptions(decimal: true),
                            decoration: InputDecoration(
                              labelText: 'Cantitate (${selectedProduct!.unit})',
                            ),
                          ),
                        ),
                        const SizedBox(width: 12),
  ...
  Future<void> _openCatalogMaterialsDialog(Appointment item) async {
    if (!mounted) return;
    // Produse din catalog (cu cache în memorie)
    final allProducts = await _getCatalogProducts();
    if (!mounted) return;

    // Copie locală a liniilor (editabilă în dialog)
    var lines = List<AppointmentMaterialUsageLine>.from(item.materialUsage.lines);

    final saved = await showDialog<bool>(
      context: context,
      builder: (dialogCtx) => StatefulBuilder(
        builder: (_, setDialogState) {
          final total = lines.fold<double>(0, (s, l) => s + l.totalCost);
          return AlertDialog(
            title: const Text('Materiale din catalog'),
            content: SizedBox(
              width: 560,
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    if (lines.isEmpty)
                      const Padding(
                        ...
                        child: Text(
                          'Nu sunt materiale adăugate. Apasă „Adaugă" pentru a alege din catalog.',
                        ),
                      )
                    else
                      for (var i = 0; i < lines.length; i++) ...[
                        ListTile(
                          ...
                          subtitle: Text(
                            '${lines[i].quantity.toStringAsFixed(lines[i].quantity == lines[i].quantity.roundToDouble() ? 0 : 2)} ${lines[i].unit} × ${lines[i].unitCost.toStringAsFixed(2)} RON',
                          ),
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Text(
                                '${lines[i].totalCost.toStringAsFixed(2)} RON',
                                style: const TextStyle(fontWeight: FontWeight.w600),
                              ),
                              IconButton(
                                tooltip: 'Șterge linie',
                                icon: const Icon(Icons.delete_outline, size: 20),
                                onPressed: () {
                                  final updated =
                                      List<AppointmentMaterialUsageLine>.from(lines)
                                        ..removeAt(i);
                                  setDialogState(() => lines = updated);
                                },
                              ),
                            ],
                          ),
                    ...
                    OutlinedButton.icon(
                      onPressed: () async {
                        final newLine = await _showPickProductFromCatalogDialog(allProducts);
                        if (newLine != null) {
                          setDialogState(() => lines = [...lines, newLine]);
                        }
                      },
                      icon: const Icon(Icons.add_outlined),
                      label: const Text('Adaugă material din catalog'),
                    ),
                  ],
                ),
              ),
    ...
    final updatedUsage = item.materialUsage.copyWith(lines: lines);
    final updated = item.copyWith(materialUsage: updatedUsage);
    setState(() {
      final idx = _items.indexWhere((e) => e.id == updated.id);
      if (idx >= 0) _items[idx] = updated;
      _updateFilteredCache();
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Materialele au fost salvate.')),
      );
    }
  }
  ...
  // Rând compact cu iconiță, etichetă și valoare — folosit în cardurile de context din dialoguri
  Widget _contextRow({
    required IconData icon,
  ...
  Widget build(BuildContext context) {
    _programariBuildCount++;
    _programariLog('build count=$_programariBuildCount');
    _logActiveBackgroundTasks();
    if (_loading) {
    ...
    return Scaffold(
      appBar: AppBar(
        actions: [
          if (_canManageAppointmentFinancials) ...[
            IconButton(
              tooltip: 'Consum materiale',
              onPressed: _openConsumMaterialeReport,
              icon: const Icon(Icons.inventory_2_outlined),
            ),
            IconButton(
                  ...
                  : items.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            // Calculăm O SINGURĂ DATĂ per card — evită apeluri duble
                            // (unele metode erau apelate de 2× în același card)
                            final clientRec = _clientRecordById(item.clientId);
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                      ...
                                      children: [
                                        ...
                                        if (_canManageAppointmentFinancials &&
                                          ...
                                          ActionChip(
                                            ...
                                            label: const Text('Plătește partener'),
                                            onPressed: () => _showQuickPartnerPaymentDialog(item),
                                          ),
                                        ...
                                        ActionChip(
                                          avatar: Icon(
                                            ...
                                            size: 16,
                                            color: (_photoCountByEntityId[item.id] ?? 0) > 0
                                                ? Colors.green
                                                : null,
                                          ),
                                          label: Text(
                                            (_photoCountByEntityId[item.id] ?? 0) > 0
                                                ? '${_photoCountByEntityId[item.id]} poze'
                                                : 'Poze teren',
                                          ),
                                          onPressed: () async {
                                            await _openFieldPhotosForAppointment(item);
                                            await _loadPhotoCountsAsync();
                                          },
                                        ),
                                        ActionChip(
                                          avatar: const Icon(
                                        ...
                                        if (_canManageAppointmentFinancials)
                                          ActionChip(
                                            avatar: Icon(
                                              ...
                                              size: 16,
                                              color: item.materialUsage.lines
                                                      .isNotEmpty
                                                  ? Colors.teal
                                                  : null,
                                            ),
                                            label: Text(
                                              item.materialUsage.lines.isNotEmpty
                                                  ? 'Catalog: ${item.materialUsage.lines.length} art.'
                                                  : 'Catalog materiale',
                                            ),
                                            onPressed: () =>
                                                _openCatalogMaterialsDialog(item),
                                          ),
                                        Chip(
                                          label: Text(
                                            'Prioritate: ${_priorityLabel(item.priority)}',
                                    ...
                                    const SizedBox(height: 8),
                                    // clientRec deja calculat O(1) din Map — fără Builder nested
                                    Builder(builder: (_) {
                                      final clientAddr = [
