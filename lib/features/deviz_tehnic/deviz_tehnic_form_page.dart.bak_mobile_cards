...
class _DevizTehnicFormPageState extends State<DevizTehnicFormPage> {
  ...
  Future<void> _loadSmartBillStock() async {
    try {
      ...
      // Silențios — stocul rămâne gol dacă SmartBill nu e disponibil
    }
  }
  ...
  Future<void> _loadCompanyDefaults() async {
    try {
      ...
      setState(() {
        ...
        // Înlocuiește email-ul cu numele real din profilul firmei (persoana de contact)
        final current = _intocmitDeCtrl.text.trim();
        if (current.contains('@') && profile.contactName.trim().isNotEmpty) {
          _intocmitDeCtrl.text = profile.contactName.trim();
        } else if (current.isEmpty && profile.contactName.trim().isNotEmpty) {
          _intocmitDeCtrl.text = profile.contactName.trim();
        }
      });
    } catch (_) {
  ...
  /// La document NOU: citește tipul implicit (cross-device), setează `_tipDocument`,
  /// apoi generează numărul corespunzător seriei.
  Future<void> _loadDefaultTipThenNumber() async {
    try {
      final defaultTip = await widget.repository.loadDefaultTipDocument();
      if (!mounted) return;
      setState(() => _tipDocument = defaultTip);
      await _loadNextNumber(defaultTip);
    } catch (_) {
      await _loadNextNumber();
    }
  }

  Future<void> _loadNextNumber([DevizTehnicTipDocument? tip]) async {
    try {
  ...
  /// Trimite pe email: generează PDF (dacă e nevoie) și deschide dialogul de email.
  Future<void> _sendEmail() async {
    String pdfPath = '';
    try {
      final record = _buildRecord();
      pdfPath = await DevizTehnicPdfService.export(
        repository: widget.appRepository,
        deviz: record,
      );
    } catch (_) {
      // PDF opțional — dialogul funcționează și fără
    }
    ...
    await showDialog<bool>(
      context: context,
      builder: (_) => DevizTehnicEmailDialog(
        deviz: record,
        company: company,
  ...
  /// Înregistrează documentul în Registratură.
  Future<void> _registerInRegistry() async {
    // Salvează mai întâi dacă e document nou
    ...
    // Verifică dacă e deja înregistrat
    if (_registryEntryId.isNotEmpty) {
      final ok = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Deja înregistrat'),
          content: Text(
              'Documentul este deja înregistrat în Registratură cu nr. $_registryNumber.\nDorești să înregistrezi din nou?'),
          actions: [
            TextButton(
            ...
            FilledButton(
              ...
              child: const Text('Da, înregistrează din nou'),
            ),
          ],
        ),
      );
    ...
    try {
      final svc = RegistryService(widget.appRepository);
      final entry = await svc.registerEstimate(
        estimateId: widget.existing?.id ?? '',
        documentNumber: record.numar,
        title:
            record.titlu.isNotEmpty ? record.titlu : record.tipDocument.label,
        clientId: record.clientId,
        recipientName: record.clientName,
        documentDate: record.dataEmiterii,
      );

      // Actualizează câmpurile de registratură în starea locală
      setState(() {
        _registryEntryId = entry.id;
        _registryNumber = entry.registryNumber;
        _registeredAt = entry.registeredAt;
        // Auto-schimbă statusul la "trimis" dacă era draft
        if (_status == DevizTehnicStatus.draft) {
          _status = DevizTehnicStatus.trimis;
        }
        _saving = false;
      });

      // Salvează automat record-ul cu noile câmpuri
      if (widget.existing != null) {
        final updatedRecord = _buildRecord();
        await widget.repository.save(updatedRecord);
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Înregistrat în Registratură cu nr. ${entry.registryNumber}'),
  ...
  /// Emite bon de consum în SmartBill cu materialele din deviz.
  Future<void> _emiteBonConsum() async {
    // Construiește lista materialelor (articole cu mat > 0 sau toate)
    ...
    for (final a in _articole) {
      ...
      materiale.add(BonConsumArticol(
        denumire: denumire,
        cantitate: cant,
        um: a.umCtrl.text.trim().isEmpty ? 'buc' : a.umCtrl.text.trim(),
        pretUnitar: a.mat,
      ));
    }
    ...
    if (materiale.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nu există articole cu cantitate în deviz.'),
        ),
      );
      return;
    }
    ...
    try {
      profile = await widget.appRepository.loadCompanyProfile();
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nu s-a putut accesa profilul firmei.'),
        ),
      );
      return;
    }
    ...
    if (!mounted) return;
    final record = _buildRecord();
    final titlu = record.numar.isNotEmpty
        ? '${record.tipDocument.label} ${record.numar}'
        : record.titlu.isNotEmpty
            ? record.titlu
            : record.tipDocument.label;
        ...
        actions: [
          if (!isNew) ...[
            IconButton(
              ...
              tooltip: 'Emite bon consum SmartBill',
              onPressed: _emiteBonConsum,
            ),
            IconButton(
              icon: const Icon(Icons.email_outlined),
              tooltip: 'Trimite pe email',
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...DevizTehnicTipDocument.values.map((tip) {
                          final selected = _tipDocument == tip;
                          return ChoiceChip(
                            label: Text(tip.label),
                            selected: selected,
                            onSelected: (_) {
                              setState(() => _tipDocument = tip);
                              // Regenerează numărul automat la documente noi
                              if (widget.existing == null) {
                                _loadNextNumber(tip);
                              }
                            },
                        ...
                        // Buton setare implicit (cross-device)
                        TextButton.icon(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      spacing: 6,
                      children: DevizTehnicPriceDisplay.values.map((pd) {
                        final selected = _priceDisplay == pd;
                        return ChoiceChip(
                          label: Text(pd.label),
                          selected: selected,
                          onSelected: (_) => setState(() => _priceDisplay = pd),
                        );
                      }).toList(),
                    ),
                  ],
            ...
            // ── Secțiune administrativă ────────────────────────
            _SectionCard(
              title: 'Stare document',
              children: [
                // Status rapid
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: _ProfitabilitateItem(
                            label: 'Profit %',
                            value: _totalDirect > 0
                                ? _profitVal / _totalDirect * 100
                                : 0,
                            cs: cs,
                            isPercent: true,
                          ),
                ...
                const Divider(),
                const SizedBox(height: 8),
                // Buton înregistrare Registratură + badge dacă e deja înregistrat
                Row(
                  children: [
                    Expanded(
                      child: _registryNumber.isNotEmpty
                          ? Row(
                              children: [
                                ...
                                Expanded(
                                  child: Text(
                                    'Registratură: $_registryNumber'
                                    '${_registeredAt != null ? '  •  ${DateFormat('dd.MM.yyyy').format(_registeredAt!)}' : ''}',
                    ...
                    OutlinedButton.icon(
                      onPressed: _saving ? null : _registerInRegistry,
                      icon: const Icon(Icons.receipt_long_outlined, size: 16),
                      label: Text(
                        _registryNumber.isNotEmpty
                            ? 'Re-înregistrează'
                            : 'Înregistrează',
                        style: const TextStyle(fontSize: 13),
                      ),
...
class _ProfitabilitateItem extends StatelessWidget {
  const _ProfitabilitateItem({
    required this.label,
    required this.value,
    required this.cs,
