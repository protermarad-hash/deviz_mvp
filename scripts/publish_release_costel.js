...
// Publicare MANUALĂ ProVentaris — client COSTEL COSTEA (proiect izolat)
// Proiect Firebase: proventaris-costel-costea (date complet separate de PRO TERM)
//
// Copie adaptată a scripts/publish_release.js. Diferă DOAR prin PROJECT_ID,
// BUCKET și numele artefactelor (convenția versionată din build_costel.ps1).
// Restul logicii (OAuth, upload, Firestore app_config/version_info) e identică
// — updateFirestore scrie automat în proiectul Costel prin PROJECT_ID de mai jos.
//
// ⚠️ PUBLICARE COMPLET MANUALĂ ȘI SEPARATĂ — NU se rulează automat la build-ul
//    PRO TERM. Se lansează explicit doar când vrei să publici versiunea Costel.
//
// Utilizare:
//   node scripts/publish_release_costel.js --version 1.5.2 --build 52 --notes "Fix login"
//
// Opțional (dacă vrei doar Android sau doar Windows):
//   node scripts/publish_release_costel.js --version 1.5.2 --build 52 --apk-only
//   node scripts/publish_release_costel.js --version 1.5.2 --build 52 --windows-only
//
// Cerințe: firebase login deja efectuat (token stocat local automat).
//          Utilizatorul logat trebuie să aibă acces la proventaris-costel-costea.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Configurare proiect (COSTEL) ─────────────────────────────────────────────
const PROJECT_ID     = 'proventaris-costel-costea';
const BUCKET         = 'proventaris-costel-costea.firebasestorage.app';
// OAuth client folosit de Firebase CLI (din firebase-tools/lib/api.js).
// Credențiale GENERICE ale Firebase CLI — nu sunt specifice unui proiect,
// deci funcționează identic pentru orice proiect la care userul logat are acces.
...
async function updateFirestore(token, { version, buildNumber, apkUrl, windowsUrl, notes }) {
  ...
  console.log('[firestore] ✓ app_config/version_info actualizat (proiect Costel)');
}
...
async function main() {
  ...
  if (!version || !buildNumber) {
    console.error(
      'Utilizare: node scripts/publish_release_costel.js --version 1.5.2 --build 52 --notes "Descriere"\n' +
      'Opțional:  --apk-only | --windows-only\n' +
      '\n' +
      'PUBLICARE MANUALĂ client COSTEL (proiect proventaris-costel-costea).'
    );
    process.exit(1);
  }
  ...
  console.log(`\n🚀  Publicare ProVentaris COSTEL v${version}+${buildNumber}`);
  console.log(`    Proiect:      ${PROJECT_ID}`);
  ...
  if (!doApk && !doWindows) {
    console.error('Niciunul dintre fișiere nu există. Rulează scripts/build_costel.ps1 mai întâi.');
    process.exit(1);
  }
  ...
  // Upload APK (în bucket-ul Costel — separat automat prin BUCKET de mai sus)
  if (doApk) {
    const storagePath = `app_releases/android/proventaris-costel-v${version}-build${buildNumber}.apk`;
  ...
  // Upload ZIP Windows
  if (doWindows) {
    const storagePath = `app_releases/windows/proventaris-windows-costel-v${version}-build${buildNumber}.zip`;
    await uploadFile(token, zipPath, storagePath);
    windowsUrl = await getDownloadUrl(token, storagePath);
  }

  // Actualizare Firestore (proiectul Costel, prin PROJECT_ID)
  console.log('\n[firestore] Actualizare app_config/version_info (Costel)...');
  await updateFirestore(token, { version, buildNumber, apkUrl, windowsUrl, notes });

  // Raport final
  console.log('\n✅  Publicare COSTEL completă!');
  ...
  console.log('\n    Aplicația Costel detectează update-ul la următoarea pornire.');
}
